...
# Running: python3 phylowalk_m8parser.py -i [input m8 file] -o [output file name]
#
...
parser = argparse.ArgumentParser(description = 'Get least common taxonomic ancestor for each read with multiple gsnap .m8 alignments')
...
df = pd.read_csv(args.i,sep='\t',header=None,names=['qseqid','sseqid','pident','length','mismatch','gapopen','qstart','qend','sstart','send','evalue','bitscore'])
print(date() + '  Finished reading in .m8 dataframe')

# iterate through all sseqid values to extract the GI number
sseqid_list = list(df['sseqid'])
gi_values = [i.split('|')[1] for i in sseqid_list]  # THIS IS TIME CONSUMING
...
query_seqs = list(df2.groupby('qseqid')['tax_id'].apply(set).index)  # unique list of query sequences, despite multiple alignments
print(date() + "  Gathered query sequences")
sets = df2.groupby('qseqid')['tax_id'].apply(set)  # create the set of taxIDs for each query seq ID
print(date() + "  Iterating through sets")
...
df3 = df2.join(qseqid_to_set_map.to_frame(), on='qseqid',how='left')
df4 = df3.join(set_master_map, on=0,how='left')

print(date() + "  Writing to output file")
df4.to_csv(args.o,index=False,sep="\t")
print(date() + "  COMPLETED")



# The output has the following columns:
# qseqid - query sequence ID, matches the .fasta sequence header
# sseqid - reference sequence ID, indicating where the sequence mapped in NCBI ref
# pident - percent identity
# length - length of alignment
# mismatch - number of mismatches
# gapopen - gap open penalty
# qstart - query sequence start, the base pair in the query sequence at which the alignment starts
# qend - query sequence ends, the base pair in the query sequence at which the alignment ends
# sstart - reference sequence start, the base pair in the ref seq at which the alignment starts
# send - reference sequence ed, the base pair in the ref seq at which the alignment end
# evalue - e value
# bitscore - bitscore for the alignment, primary metric for alignment quality
# raw_gi - GI value extracted from the sseqid field (primarily for debugging purposes and tracability of results)
# tax_id - TaxID value to which the GI values mapped
# 0 - the list of all TaxIDs that this query sequence mapped to, note: if a query sequence had multiple alignments, there will be multiple taxIDs but the list will be the same for every entry of that line
# LCA - TaxID of LCA
# LCAclass - the class corresponding to the TaxID of LCA (ie genus, phylum, kingdom)
